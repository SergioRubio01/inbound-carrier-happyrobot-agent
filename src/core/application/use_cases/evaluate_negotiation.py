...
class EvaluateNegotiationUseCase:
    ...
    def __init__(self,
                 load_repository: ILoadRepository,
                 carrier_repository: ICarrierRepository):
        self.load_repository = load_repository
        self.carrier_repository = carrier_repository

    async def execute(self, request: EvaluateNegotiationRequest) -> EvaluateNegotiationResponse:
        ...
        try:
            ...
            if not load:
                raise NegotiationEvaluationException(f"Load {request.load_id} not found")
            ...
            if not carrier:
                raise NegotiationEvaluationException(f"Carrier {request.mc_number} not found")
            ...
            raise NegotiationEvaluationException(f"Failed to evaluate negotiation: {str(e)}")
    ...
    def _calculate_history_factor(self, carrier, context: Optional[Dict[str, Any]]) -> float:
    ...
    def _create_accepted_response(self, negotiation: Negotiation, load) -> EvaluateNegotiationResponse:
    ...
    def _create_counter_offer_response(self, negotiation: Negotiation) -> EvaluateNegotiationResponse:
        ...
        return EvaluateNegotiationResponse(
            ...
            load_id=str(negotiation.load_id),
            carrier_offer=negotiation.carrier_offer.to_float(),
            counter_offer=negotiation.counter_offer.to_float() if negotiation.counter_offer else None,
            negotiation_round=negotiation.round_number,
    ...
    def _create_rejected_response(self, negotiation: Negotiation) -> EvaluateNegotiationResponse:
        ...
        return EvaluateNegotiationResponse(
            ...
            load_id=str(negotiation.load_id),
            carrier_offer=negotiation.carrier_offer.to_float(),
            maximum_rate=negotiation.maximum_acceptable.to_float() if negotiation.maximum_acceptable else None,
            negotiation_round=negotiation.round_number,
            message=negotiation.message_to_carrier,
            timestamp=datetime.utcnow()
        )
