...
def downgrade():
    ...
    with op.batch_alter_table('loads') as batch_op:
        ...
        batch_op.add_column(sa.Column('origin_coordinates', postgresql.JSONB, nullable=True))
        batch_op.add_column(sa.Column('origin_facility', postgresql.JSONB, nullable=True))
        batch_op.add_column(sa.Column('destination_coordinates', postgresql.JSONB, nullable=True))
        batch_op.add_column(sa.Column('destination_facility', postgresql.JSONB, nullable=True))

        # Re-add extended schedule fields
        batch_op.add_column(sa.Column('pickup_time_end', sa.Time(), nullable=True))
        batch_op.add_column(sa.Column('pickup_appointment_required', sa.Boolean(), default=False))
        batch_op.add_column(sa.Column('delivery_time_end', sa.Time(), nullable=True))
        batch_op.add_column(sa.Column('delivery_appointment_required', sa.Boolean(), default=False))

        # Re-add extended equipment fields
        batch_op.add_column(sa.Column('equipment_requirements', postgresql.JSONB, nullable=True))

        # Re-add unauthorized load detail fields
        batch_op.add_column(sa.Column('pieces', sa.Integer(), nullable=True))
        batch_op.add_column(sa.Column('commodity_description', sa.Text(), nullable=True))
        ...
        batch_op.add_column(sa.Column('miles', sa.Integer(), nullable=False, server_default='0'))
        batch_op.add_column(sa.Column('estimated_transit_hours', sa.Integer(), nullable=True))
        ...
        batch_op.add_column(sa.Column('auto_accept_threshold', sa.NUMERIC(10, 2), nullable=True))

        # Re-add broker/customer information
        batch_op.add_column(sa.Column('broker_company', sa.String(255), nullable=True))
        batch_op.add_column(sa.Column('broker_contact', postgresql.JSONB, nullable=True))
        batch_op.add_column(sa.Column('customer_name', sa.String(255), nullable=True))

        # Re-add extended status fields
        batch_op.add_column(sa.Column('status_changed_at', sa.TIMESTAMP(timezone=True), server_default=sa.func.now()))
        batch_op.add_column(sa.Column('booked_by_carrier_id', postgresql.UUID(as_uuid=True), nullable=True))
        batch_op.add_column(sa.Column('booked_at', sa.TIMESTAMP(timezone=True), nullable=True))

        # Re-add extended instruction fields
        batch_op.add_column(sa.Column('special_requirements', postgresql.ARRAY(sa.Text()), nullable=True))
